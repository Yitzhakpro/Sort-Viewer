...
import type { ColorMapping, IdentifiedNumber, IdentifiedNumberList, SortSteps } from '../types';
...
function quickSort(
	...
	if (left < right) {
		...
		if (right >= 0) {
			...
			steps.push(
				createSortStep(arr, {}, { ...steps.at(-1)?.permanentColorMapping, ...sortedMapping })
			);
		}
	}
}

export function quickSortWithSteps(arr: IdentifiedNumberList): SortSteps<IdentifiedNumber> {

...
export function insertionSortWithSteps(arr: IdentifiedNumberList): SortSteps<IdentifiedNumber> {
	...
	for (let i = 1; i < arrSize; i++) {
		...
		insertionSortSteps.push(createSortStep(arr, { [i]: 'CHECK', [j]: 'CHECK' }, {}));

		while (j >= 0 && arr[j].number > currentElement.number) {
			insertionSortSteps.push(createSortStep(arr, { [j]: 'SWAPPING', [j + 1]: 'SWAPPING' }, {}));
			const tmp = arr[j];
			arr[j] = arr[j + 1];
			arr[j + 1] = tmp;
			insertionSortSteps.push(createSortStep(arr, { [j]: 'SWAPPING', [j + 1]: 'SWAPPING' }, {}));
